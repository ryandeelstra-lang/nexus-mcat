...
"""Generate authored SpokenPrompt variants for the seed corpus (voice-Keeper spec §9 / doc 24 AF-7).
...
any variant that reveals the answer's content words.

Usage: ANTHROPIC_API_KEY=... out/pyenv/bin/python scripts/gen_spoken_variants.py [--leaf PS.8A]
"""
...
BATCH = 20  # cards per Claude call — big enough to be cheap, small enough to stay reliable

sys.path.insert(0, str(ROOT))
from journey.voice_review import _front_hash  # noqa: E402  the ONE normalization (never duplicate)
...
def reword_batch(client, cards: list[dict]) -> list[str]:  # type: ignore[no-untyped-def]
    ...
    msg = client.messages.create(
        ...
        max_tokens=4000,
        temperature=0.4,  # some variety in surface form; the answer contract is in the prompt
        system=SYSTEM,
    ...
    if not isinstance(prompts, list) or len(prompts) != len(cards):
        got = len(prompts) if isinstance(prompts, list) else type(prompts)
        raise ValueError(f"expected {len(cards)} prompts, got {got}")
...
def leaked(prompt: str, back: str) -> bool:
    ...
    b, p = content_words(back), content_words(prompt)

...
import { allAssetKeys, applyDisplaySize, DISPLAY, ensureTexture, regionThemeFromSection, stageTextureKey } from "../assets";
...
export class CinematicScene extends Phaser.Scene {
    ...
    private renderGround(): void {
        for (const r of this.plan.regions) {
            ...
            for (const t of r.waterTiles) {
                ...
                this.trimmedTile(t.tileX, t.tileY, `tile-${pondTheme}-pond-${String(v).padStart(2, "0")}`, t.tileY + 0.1);
            }
    ...
    private renderStructures(): void {
        ...
        const lanternSpots = [
            { x: 55, y: 40 }, { x: 65, y: 40 }, { x: 55, y: 50 }, { x: 65, y: 50 },
            { x: 50, y: 45 }, { x: 70, y: 45 },
        ];
    ...
    private renderPlants(): void {
        const addPlant = (tileX: number, tileY: number, region: string, seed: number): void => {
            ...
            const spr = this.add.image(tileX * TS + TS / 2, tileY * TS + TS, ensureTexture(this, stageTextureKey("bloomed")));
            spr.setOrigin(0.5, 1);
    ...
    private renderWorldFx(): void {
        ...
        for (let i = 0; i < 10; i++) {
            const spr = this.add.image(hero.x, hero.y, ensureTexture(this, `fx-droplet-${String(i % 9).padStart(2, "0")}`));
    ...
    private applyStructures(t: number): void {
        ...
        if (this.southGate.texture.key !== southKey) {
            const src = this.textures.get(ensureTexture(this, southKey)).getSourceImage() as { width: number; height: number };

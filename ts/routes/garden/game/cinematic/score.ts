...
import { midiToFreq, REGION_PROFILES, type Chord } from "../../audio/theory";
...
const SEGMENTS: readonly SegmentMix[] = [
    ...
    { from: BEATS.careSakura.from, pad: 0.55, keys: 0.5, bass: 0.6, drums: 0.45, lead: 0.5, vinyl: 0.32, cutoffHz: 2600 },
...
function progressionAt(t: number): { chords: Chord[]; leadScale: number[]; leadOctave: number; leadVoice: "pluck" | "bell" | "marimba"; leadDensity: number } {
    if (t < BEATS.careSakura.from) {
        const p = REGION_PROFILES.keukenhof; // bright, hopeful awe
        return { chords: p.progression, leadScale: p.leadScale, leadOctave: p.leadOctave, leadVoice: "marimba", leadDensity: 0.3 };
    }
    if (t < BEATS.peakVersailles.from) {
        const p = REGION_PROFILES.sakura; // the koto care vignette
        return { chords: p.progression, leadScale: p.leadScale, leadOctave: p.leadOctave, leadVoice: "pluck", leadDensity: 0.55 };
    }
    if (t < BEATS.departure.from) {
        const p = REGION_PROFILES.versailles;
        return { chords: p.progression, leadScale: p.leadScale, leadOctave: p.leadOctave, leadVoice: "marimba", leadDensity: 0.5 };
    }
    if (t < BEATS.student.from) {
        const p = REGION_PROFILES["gardens-by-the-bay"]; // contemplative dusk + sleep
        return { chords: p.progression, leadScale: p.leadScale, leadOctave: p.leadOctave, leadVoice: "bell", leadDensity: t < BEATS.longSleep.from ? 0.35 : 0.15 };
    }

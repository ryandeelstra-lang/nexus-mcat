...
function sampleDT(dt: Float32Array, gw: number, gh: number, px: number, py: number): number {
...
export function buildTerrainModel(plan: WorldPlan): TerrainModel {
    ...
    for (let gy = 0; gy < gh; gy++) {
        for (let gx = 0; gx < gw; gx++) {
            ...
            regionOfCell[gy * gw + gx] = east ? (south ? 3 : 1) : (south ? 2 : 0);
        }
    }
...
const PALETTES: GroundPalette[] = [
    {
        ...
        pathRim: rgb("#98805A"),
        waterDeep: rgb("#17606F"),
        water: rgb("#2B8497"),
        waterLight: rgb("#5FB4C4"),
        shore: rgb("#CBB289"),
    },
    {
    ...
    {
        grass: [rgb("#27412F"), rgb("#213A29"), rgb("#1B3123")],
        tuft: rgb("#12241A"),
        flowers: [rgb("#59D6BC"), rgb("#9D5DE0"), rgb("#3FB3D9")],
        flowerDensity: 0.014,
        pebble: rgb("#3E4A41"),
...
const PLAZA_RX = 8.5 * TILE;
const PLAZA_RY = 6 * TILE;
        ...
        for (let cx0 = 0; cx0 < wpx; cx0 += CHUNK) {
            ...
            for (let by = 0; by < ch; by += BLOCK) {
                for (let bx = 0; bx < cw; bx += BLOCK) {
                    ...
                    let color: RGB;
                    const waterW = TILE * 0.72 + (fbm(px * 0.01, py * 0.01, 31) - 0.5) * TILE * 0.5;
                    ...
                    if (wd + edgeJ < waterW) {
                        ...
                        if (pe < 1) {
                            ...
                            if (td + edgeJ < pathW) {
                                ...
                                const shade = pal.grass[band < 0.42 ? 2 : band < 0.58 ? 1 : 0];
                                const micro = hash2(px, py, 81);
                                const m = (micro - 0.5) * 14;
                                ...
                                if (micro > 0.94 && clus > 0.52) {
                                    ...
                                    hash2(px, py, 99)
                                        < pal.flowerDensity * (clus > 0.62 ? 2.5 : 0.35)
                                ) {
                                    color = pal.flowers[
                                        (hash2(px, py, 100) * pal.flowers.length) | 0
                                    ];
                                } else if (hash2(px, py, 111) < 0.005) {
                                    color = pal.pebble;
                                }
                            }
                        }
...
const DECOR: Record<number, RegionDecor> = {
    ...
    3: {
        ...
        small: [
            fam("foliage-gardens-by-the-bay", [16, 18, 19, 25, 27, 28, 29, 30, 31, 32], [0.8, 1.2]),
        ],
    },
};
...
const VERSAILLES_TOPIARY = ["foliage-versailles-00", "foliage-versailles-03", "foliage-versailles-10", "foliage-versailles-12"];
const VERSAILLES_STATUES = ["prop-versailles-r0-02", "prop-versailles-r0-04", "prop-versailles-r0-10", "prop-versailles-r0-14"];
...
export function planDecor(
    ...
    for (const r of plan.regions) {
        ...
        for (const p of r.props) {
            blockPts.push({ x: p.tileX + 0.5, y: p.tileY + 0.5, r: 2.4 });
        }
    ...
    const clearAt = (tx: number, ty: number, size: number): boolean => {
        const px = tx * TILE;
        const py = ty * TILE;
        if (px < 8 || py < 8 || px > plan.widthTiles * TILE - 8 || py > plan.heightTiles * TILE - 8) {
            return false;
        }
    ...
    const regionAtTile = (tx: number, ty: number): number => (tx > 60 ? (ty > 45 ? 3 : 1) : (ty > 45 ? 2 : 0));

    // A. Border forest bands — map edge + the seams between regions (Champion-Island framing).
    for (let ty = 1; ty < plan.heightTiles - 1; ty++) {
        for (let tx = 1; tx < plan.widthTiles - 1; tx++) {
            const onEdge = tx < 4 || tx >= plan.widthTiles - 4 || ty < 4 || ty >= plan.heightTiles - 4;
    ...
    // B. Interior scatter per region — medium clumps then small fillers, species clustered.
    for (const r of plan.regions) {
        ...
        for (let ty = r.rect.y + 2; ty < r.rect.y + r.rect.h - 1; ty += 3) {
            for (let tx = r.rect.x + 2; tx < r.rect.x + r.rect.w - 1; tx += 3) {
                ...
                const family = decor.medium[(clus * decor.medium.length) | 0]
                    ?? decor.medium[0];
...
function setPieces(
    ...
    if (bb && TULIP_STRIPS.some(exists)) {
        ...
        for (const p of patches) {
            ...
            for (let y = p.y0 + hStrip; y <= p.y1; y += hStrip * 0.85) {
                ...
                // Skip rows that cross water/paths.
                let blocked = false;
    ...
    // Versailles: render the (already-solid) hedge rows + formal topiary along the allées.

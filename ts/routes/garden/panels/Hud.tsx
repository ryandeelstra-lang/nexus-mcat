...
export function Hud(props: HudProps): React.ReactElement {
    const { balances, tutorial, growthLine, onOpenAlmanac, onOpenMapHelp, onToggleMap } = props;
    const [sky, setSky] = useState<SkyDialState>(() => skyDialStateFor(new Date()));
    const [reducedMotion, setReducedMotion] = useState(false);
    const [waterBump, setWaterBump] = useState(false);
    const [seedBump, setSeedBump] = useState(false);
    ...
    useEffect(() => {
        ...
        if (balances.seeds !== lastBalances.current.seeds) {
            setSeedBump(true);
            globalThis.setTimeout(() => setSeedBump(false), 180);
        }
    ...
    const chipSeedClass = [
        "hud-chip",
        !reducedMotion && seedBump ? "hud-bump" : "",
    ].filter(Boolean).join(" ");

    return (
        <div className="garden-hud" aria-label="Garden heads-up display">
            <div className="hud-top hud-top-left">
                <span className={chipWaterClass}>💧 {balances.water}</span>
                <span className={chipSeedClass}>🌱 {balances.seeds}</span>
            </div>
            ...
            <div className="hud-top hud-top-right">
                ...
                <button className="hud-ghost-button" onClick={onToggleMap}>
                    Map
                </button>
                <button className="hud-ghost-button" onClick={onOpenMapHelp}>
                    ?
                </button>
                <div className="hud-dial" aria-label="Day and night dial">
                    <svg viewBox="0 0 40 20" role="img" aria-hidden="true">

...
// charged_up: the voice Keeper reviewer (voice-Keeper spec §3) — REPLACES the reveal +
// 1-4 self-grade. The Keeper asks (a reworded variant when authored), you speak or type,
// the server grades and applies the real answerCard. The classic StudyCard remains ONLY
// as the escape-hatch fallback (CHARGED_UP_VOICE_REVIEWS=0). Controls live in the React
// chrome, never inside the sandboxed card iframe (opaque origin by design — no Bearer
// header, no mic permission in there).
...
export interface VoiceGradedEvent {
    /** Client CardAnswer_Rating (0-3), already converted from the server's 1-4. */
    rating: number;
    msTaken: number;
    /** null on the classic-fallback path (the flat +1 economy applies there). */
...
export interface VoiceStudyCardProps {
    ...
    onClose(): void;
    /** Honest skip: no reworded variant exists for this scope (the bloom beat only). */
    onNoVariant?(): void;
    contextLabel?: string;
    /** A one-shot key that forces a fresh session when the outer scope (deck) changes. */
    scopeKey: string;
    /** Serve only a card with a reworded variant (the ProveIt bloom check). */
    preferVariant?: boolean;
    /** One card, then onEmpty — the ProveIt beat. */
    singleCard?: boolean;
}
...
const BUCKET_BEAT: Record<VoiceBucket, { title: string; icon: string; tone: string }> = {
    ...
    dont_know: { title: "That's okay — we'll water it.", icon: "💧", tone: "voice-beat-idk" },
};
        ...
        <div className="keeper-panel" role="dialog" aria-label="The Keeper's questions">
            ...
            {(answerable || busy) && (
                <>
                    ...
                    {state.micError && (
                        <div className="keeper-status voice-mic-error" role="alert">
                            {state.micError}
                        </div>
                    )}
                            ...
                            <div className="keeper-actions voice-answer-row">
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        disabled={!typed.trim() || state.phase === "listening"}
                                    >
                                        Answer
                                    </button>
                                </form>
            ...
            {state.phase === "result" && state.result && (
                ...
                >
                    ...
                    <p className="voice-transcript">You said: “{state.result.transcript}”</p>
                    ...
                    {state.result.sentinel && (
                        <p className="voice-sentinel" role="note">{state.result.sentinel}</p>
                    )}
                    <div className="keeper-actions">
                        <button className="keeper-reveal" onClick={() => void review.advance()}>
                            Continue <kbd>Enter</kbd>
                        </button>
                        <button className="voice-appeal" onClick={() => review.appeal()}>

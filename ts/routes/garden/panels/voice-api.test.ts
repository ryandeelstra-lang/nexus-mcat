...
// charged_up: pins the voice-review client contract (voice-Keeper spec §4) — snake_case
// server payloads map to camelCase types, and the server's v3 ease (1-4) converts to the
// client CardAnswer_Rating enum (0-3) in exactly one place.
...
describe("fetchNextVoiceCard", () => {
    it("maps a served card payload", async () => {
        ...
        if (res.kind === "card") {
            ...
            expect(res.card.keeperLine).toBe("What is self-concept?");
    ...
    it("maps done and noVariant", async () => {
        ...
        expect((await fetchNextVoiceCard({ preferVariant: true })).kind).toBe("noVariant");
    });

    it("maps a missing backend to unavailable", async () => {
...
describe("gradeVoiceAnswer", () => {
    it("maps a terminal grade with the rating converted", async () => {
        ...
        if (out.kind === "graded") {
            ...
            expect(out.result.bucket).toBe("good");
            expect(out.result.bloomed).toBe(true);
            expect(out.result.correctAnswer).toBe("Self-concept — ...");
        }
    });

    it("maps a re-prompt (no commit yet)", async () => {
        mockFetchOnce({
            available: true,
            applied: false,
            bucket: "ask_again",
    ...
    it("maps stt errors and server errors honestly", async () => {
        mockFetchOnce({ available: true, applied: false, stt_error: "local STT failed: x" });

...
describe("onVoiceGradedAnswer — voice-Keeper spec ruling 2 (+3/+2/+2 recovered/+1)", () => {
    ...
    it("still grants xp per graded answer", () => {
        expect(onVoiceGradedAnswer(base, "good", false).xp).toBe(1);
    });

    it("the reward table matches the doc 24 §3 contract", () => {
        expect(VOICE_WATER_REWARD).toEqual({ good: 3, okay: 2, ask_again: 1, dont_know: 1 });
    });
});
